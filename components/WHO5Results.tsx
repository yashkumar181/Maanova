...
// SIMPLIFIED: Removed isSaved and setIsSaved from the interface.
...
export function WHO5Results({ responses, questions, onRestart }: WHO5ResultsProps) {
  ...
  useEffect(() => {
    // SIMPLIFIED: The check is now much simpler.
    // The lock inside saveProgressResponse will handle the duplicate prevention.
    if (user && totalScore > 0) { 
      saveProgressResponse(user.uid, "WHO5", totalScore, responses);
    }
  }, [user, totalScore, responses]); // Simplified dependency array
  ...
  return (
    <main className="min-h-screen bg-background p-4">
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="text-center mb-8"><h1 className="text-3xl font-bold mb-2">Your WHO-5 Results</h1><p className="text-muted-foreground">Based on your responses from the past two weeks</p></div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center"><Button onClick={onRestart} variant="outline"><RotateCcw className="h-4 w-4 mr-2" />Retake Assessment</Button><Button>Book Counseling Session</Button><Button variant="outline">View Resources</Button></div>
        <Card className="bg-muted/50"><CardContent className="pt-6"><p className="text-xs text-muted-foreground text-center"><strong>Disclaimer:</strong> This assessment is for informational purposes only and is not a substitute for professional medical advice, diagnosis, or treatment. If you're experiencing severe mental health symptoms, please contact a healthcare professional or crisis helpline immediately.</p></CardContent></Card>
      </div>
    </main>
  );
}